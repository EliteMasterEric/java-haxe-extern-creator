...
public class ClassFormaterCallable implements Callable<Void> {
		...
		for (List<MethodDoc> list : methodsByName.values()) {
			...
			for (MethodDoc method : overload2) {
				if (method.overriddenMethod() != null) {
					// some overloaded methods also require overriding but their overridenMethod
					// returns null
					// by identifying the methods marked as OVERLOADEDv2 that do require overrides
					// we can
	...
	private String fieldString(FieldDoc field) {
		...
		if (constValue == null && !(constValue instanceof Integer || constValue instanceof String
				|| constValue instanceof Float || constValue instanceof Byte))
			s = String.format("%svar %s:%s;", modifierString, Stringifier.reservedNameAvoid(field.name()),
					Stringifier.typeToString(field.type()));
		else {
			String formatString;
			...
			s = String.format(formatString, modifierString, Stringifier.reservedNameAvoid(field.name()),
					Stringifier.typeToString(field.type()), constValue);
		}
	...
	private String constructorString(ConstructorDoc constructor, MethodStringType type) {
		...
		for (Parameter param : constructor.parameters()) {
			parameterString += String.format("%s:%s,", Stringifier.reservedNameAvoid(param.name()),
					Stringifier.typeToString(param.type()));
		}
		...
		switch (type) {
			...
			case OVERLOADEDv3:
			case OVERLOADEDv2:
				s = String.format("%soverload function new(%s):Void;", modifierString, parameterString);
				break;
		}
		return s;
	}
	...
	private String methodString(MethodDoc method, MethodStringType type) {
		...
		switch (type) {
			case NORMAL:
				s = createMethodString(method, parameterString, returnString, false, false);
				break;
			case OVERLOADEDv3:
			case OVERLOADEDv2:
				s = createMethodString(method, parameterString, returnString, false, true);
				break;
			case REQUIRES_OVERRIDE:
				s = createMethodString(method, parameterString, returnString, true, false);
				break;
		}
		return s;
	}
	...
	private String createMethodString(MethodDoc method, String parameterString, String returnString,
			Boolean requiresOverride, Boolean requiresOverload) {
		String s;
		String modifierString = "";
